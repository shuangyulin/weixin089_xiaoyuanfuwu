	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.leixingmingcheng" type="text" placeholder="类型名称" ></input>
			</view>
			<view class="action">
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
					...
					<text v-if="isAuth('leixing','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('leixing','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"rgba(255, 153, 0, 1)","borderColor":"rgba(255, 255, 0, 1)","borderRadius":"8rpx","color":"rgba(255, 255, 255, 1)","borderWidth":"1","width":"80%","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('leixing','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.leixingmingcheng="";
			},
			//类别搜索
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
												...
												if(this.searchForm.leixingmingcheng){
					searchForm['leixingmingcheng'] = '%' + this.searchForm.leixingmingcheng + '%'
				}	
												let res = await this.$api.list(`leixing`, searchForm);
				// 如果是第一页数据置空
