<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(255, 153, 0, 1)","textAlign":"left"}' class="title">类型名称</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(255, 153, 0, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.leixingmingcheng" v-model="ruleForm.leixingmingcheng" placeholder="类型名称"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if(options.cross){
				...
				for (var o in obj){
					if(o=='leixingmingcheng'){
					this.ruleForm.leixingmingcheng = obj[o];
					this.ro.leixingmingcheng = true;
					continue;
					}
				}
			}
		...
		methods: {
			...
			async onSubmitTap() {
																																																																if((!this.ruleForm.leixingmingcheng)){
					this.$utils.msg(`类型名称不能为空`);
					return
				}
																																								if(this.ruleForm.id){
